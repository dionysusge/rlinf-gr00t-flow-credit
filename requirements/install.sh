...
try:
    installed = version("mani_skill")
except PackageNotFoundError:
    installed = None

print(
    "[install_maniskill_libero_env] "
    f"existing mani_skill={installed}"
)

raise SystemExit(
    0 if installed == "3.0.0b22" else 1
)
PYMS
    then
        echo "[install_maniskill_libero_env] Exact ManiSkill version already installed."
    elif uv pip install --offline \
        "git+https://github.com/haosulab/ManiSkill.git@v3.0.0b22"
    then
        echo "[install_maniskill_libero_env] Restored ManiSkill from uv cache."
    else
        echo "[install_maniskill_libero_env] Offline cache unavailable; using PyPI mirror."
        uv pip install "mani_skill==3.0.0b22"
    fi
...
print(
    "[install_maniskill_libero_env] "
    f"verified mani_skill={installed}"
)
assert installed == "3.0.0b22", installed
PYMSVERIFY
